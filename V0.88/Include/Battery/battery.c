...
void battery_voltage_init(void)
{
    ...
    nrf_saadc_channel_config_t config = NRF_DRV_SAADC_DEFAULT_CHANNEL_CONFIG_SE(Gain_Battery);  //NRF_SAADC_INPUT_VDD
    err_code = nrf_drv_saadc_channel_init(2, &config);                                                                  // era valor 0
    APP_ERROR_CHECK(err_code);
    battery_is_init = 1;
}
